...
class CodeWriter:
    ...
    def _write_header_to_file(self):
        self._open_file_to_write_if_not_opened()
        self.open_file.writelines(f"// ASM FILE created by VMTranslator created by pajdek.\n"
    ...
    def _raise_unrecognised_cmd(cmd):
        raise UnrecognisedCmdError(f"{cmd} is not handled by the compiler, check your VM code")
    ...
    def _generate_gt_cmd(self):
        ...
        command_lines = ('\n// gt', '@SP', 'AM=M-1', 'D=M', '@R13', 'M=D', '@SP', 'A=M-1', 'D=M', '@R13', 'D=D-M',
                         f'@gt{label}', 'D; JGT', '@SP', 'A=M-1', 'M=0', f'@gtEND{label}', '0; JMP', f'(gt{label})',
                         '@SP', 'A=M-1', 'M=-1',  f'(gtEND{label})')
        return '\n'.join(command_lines)
    ...
    def _generate_lt_cmd(self):
        ...
        command_lines = ('\n// gt', '@SP', 'AM=M-1', 'D=M', '@R13', 'M=D', '@SP', 'A=M-1', 'D=M', '@R13', 'D=D-M',
                         f'@lt{label}', 'D; JLT', '@SP', 'A=M-1', 'M=0',
                         f'@ltEND{label}', '0; JMP', f'(lt{label})', '@SP', 'A=M-1', 'M=-1', f'(ltEND{label})')
        return '\n'.join(command_lines)
    ...
    def _generate_eq_cmd(self):
        ...
        command_lines = ('\n// gt', '@SP', 'AM=M-1', 'D=M', '@R13', 'M=D', '@SP', 'A=M-1', 'D=M', '@R13', 'D=D-M',
                         f'@eq{label}', 'D; JEQ', '@SP', 'A=M-1', 'M=0',
                         f'@eqEND{label}', '0; JMP', f'(eq{label})', '@SP', 'A=M-1', 'M=-1', f'(eqEND{label})')
        return '\n'.join(command_lines)

    @staticmethod
    def _generate_push_constant_cmd(cmd):
        command_lines = (f'\n//{cmd}', f"@{cmd.arg_2}", "D=A", '@SP', 'A=M', 'M=D', '@SP', 'M=M+1')
        return '\n'.join(command_lines)

    @staticmethod
    def _generate_pop_local_cmd(cmd):
        command_lines = (f'\n//{cmd}', f"@{cmd.arg_2}", "D=A", "@LCL", "D=M+D", '@R13', 'M=D',
    ...
    def _generate_push_cmd_for_local_argument_this_that(cmd):
        ...
        command_lines = (f'\n//{cmd}', f"@{cmd.arg_2}", "D=A", f"@{pointer_mapping[cmd.arg_1]}", "A=M+D", 'D=M',
                         '@SP', 'A=M', 'M=D', '@SP', 'M=M+1')
        return '\n'.join(command_lines)
    ...
    def _generate_pop_cmd_for_local_argument_this_that(cmd):
        ...
        command_lines = (f'\n//{cmd}', f"@{cmd.arg_2}", "D=A", f"@{pointer_mapping[cmd.arg_1]}", "D=M+D",
    ...
    def _generate_push_temp_cmd(cmd):
        temp_start = 5
        command_lines = (f'\n//{cmd}', f"@{temp_start + cmd.arg_2}", 'D=M', '@SP', 'A=M', "M=D", '@SP', 'M=M+1')
        return '\n'.join(command_lines)

    @staticmethod
    def _generate_pop_temp_cmd(cmd):
        temp_start = 5
        command_lines = (f'\n//{cmd}', '@SP', 'AM=M-1', 'D=M', f"@{temp_start + cmd.arg_2}", "M=D")
        return '\n'.join(command_lines)

    def _generate_push_static_cmd(self, cmd):
        command_lines = (f'\n//{cmd}', f"@{self.file_name}.{cmd.arg_2}", 'D=M', '@SP', 'A=M', "M=D", '@SP', 'M=M+1')
        return '\n'.join(command_lines)

    def _generate_pop_static_cmd(self, cmd):
        command_lines = (f'\n//{cmd}', '@SP', 'AM=M-1', 'D=M', f"@{self.file_name}.{cmd.arg_2}", "M=D")
        return '\n'.join(command_lines)
    ...
    def _generate_push_pointer_cmd(cmd):
        ...
        command_lines = (f'\n//{cmd}', f"@{pointer_mapping[cmd.arg_2]}", 'D=M', '@SP', 'A=M', "M=D", '@SP', 'M=M+1')
        return '\n'.join(command_lines)
    ...
    def _generate_pop_pointer_cmd(cmd):
        ...
        command_lines = (f'\n//{cmd}', '@SP', 'AM=M-1', 'D=M', f"@{pointer_mapping[cmd.arg_2]}", "M=D")
        return '\n'.join(command_lines)
