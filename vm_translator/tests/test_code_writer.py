...
def test_code_writer_init(mockdata_time):
    ...
    expected_file_header = '// ASM FILE created by VMTranslator created by pajdek.\n' \
                           '// Compilation date: 2023-01-17 15:01:16'
    mock_file = Path('tmp_path/mocked.asm')

    with patch('builtins.open') as mocked_open:
        code_writer = CodeWriter(mock_file)

        assert code_writer.module_name == expected_module_name
        assert code_writer.label_counter == expected_label_counter
        mocked_open.assert_called_once_with(mock_file, 'w')

        mocked_open().writelines.assert_called_once_with(expected_file_header)
...
@pytest.mark.parametrize('command, asm_expected_code', [
